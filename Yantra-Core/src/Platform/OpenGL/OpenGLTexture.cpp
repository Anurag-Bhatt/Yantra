#include "OpenGLTexture.h"
#include "Yantra/Core.h"
#include <yqpch.h>

#include "Yantra/Log.h"
#include <glad/glad.h>
#include <stb_image.h>

namespace Yantra {

OpenGLTexture::OpenGLTexture(const std::string &path) : m_Path(path) {
  int32 width, height, channels;

  stbi_set_flip_vertically_on_load(1);

  stbi_uc *image_data = stbi_load(path.c_str(), &width, &height, &channels, 0);

  if (image_data) {

    m_Width = width;
    m_Height = height;

    GLenum internalFormat = 0, dataFormat = 0;
    if (channels == 4) {
      internalFormat = GL_RGBA8;
      dataFormat = GL_RGBA;
    } else if (channels == 3) {
      internalFormat = GL_RGB8;
      dataFormat = GL_RGB;
    }

    m_InternalFormat = internalFormat;
    m_DataFormat = dataFormat;

    YANTRA_CORE_ASSERT(internalFormat & dataFormat, "Format not supported!");
    if (internalFormat & dataFormat) {

      // Configure Textures on GPU
      glCreateTextures(GL_TEXTURE_2D, 1, &m_RendererID);
      glTextureStorage2D(m_RendererID, 1, internalFormat, m_Width, m_Height);

      // Configure Filtering
      glTextureParameteri(m_RendererID, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
      glTextureParameteri(m_RendererID, GL_TEXTURE_MAG_FILTER, GL_NEAREST);

      glTextureParameteri(m_RendererID, GL_TEXTURE_WRAP_S, GL_REPEAT);
      glTextureParameteri(m_RendererID, GL_TEXTURE_WRAP_T, GL_REPEAT);

      glTextureSubImage2D(m_RendererID, 0, 0, 0, m_Width, m_Height, dataFormat,
                          GL_UNSIGNED_BYTE, image_data);

      YANTRA_CORE_INFO("OpenGLTexture: Successfully loaded Texture! ({0}{1})",
                       width, height);
    } else {
      YANTRA_CORE_ERROR("OpenGLTexture: Format not supported! channels {0}",
                        channels);
    }
    stbi_image_free(image_data);
  } else {
    YANTRA_CORE_ERROR(
        "OpenGLTexture: Failed to load image!, stbi_failure_reason: {0}",
        stbi_failure_reason());
  }
}

OpenGLTexture::~OpenGLTexture() { glDeleteTextures(1, &m_RendererID); }

void OpenGLTexture::Bind(uint32 slot) const {
  glBindTextureUnit(slot, m_RendererID);
}

} // namespace Yantra
